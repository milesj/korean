...
On the other hand, the `에서` particle is used for:

- Referencing a dynamic location in which an activity is happening. This is typically paired with an
  action verb.
- Marking the source location of a movement, the "from", to a destination location, the "to" via
  [`까지`](../to).
